...
class BPNN:
    ...
    def train_v2(self, data, labels, samples_length, epoches, batch_size, train_set_sample_ids, test_set_ids,
                 ...
                 log_dir='./data/log/bpnn_models', random_seed=None):
        ...
        for i in range(epoches):
            ...
            print()
            # self.sess.run(self.accuracy, feed_dict={})
        bpnn_info = {'batch_loss': b_losses, 'batch_accuracy': b_aces, 'val_accuracy': val_aces}
        with open('./data/info/bpnn_info.json', 'w') as f:
            json.dump(bpnn_info, f)
    ...
    def test_time(self, data, labels, sample_length, test_set_ids, save_dir='./data/confusion_matrix/bpnn'):
        whole_time, y_true, y_predict = self._get_test_result_and_running_time(data, labels, sample_length, 1024,
                                                                               test_set_ids)
        np.savez(save_dir + '/bpnn_predict.npz', y_predict=y_predict, y_true=y_true)
        return whole_time
